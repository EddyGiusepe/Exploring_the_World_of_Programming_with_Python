...
OPENAI_API_KEY = Eddy_key_openai
...
st.markdown("<h1 style='text-align: center; color: blue;'>🤖 Personal AI Assistant</h1>", unsafe_allow_html=True)
st.sidebar.markdown("<h3 style='text-align: center; color: blue;'>Assistant Console</h3>", unsafe_allow_html=True)
...
data_source = st.selectbox("Sobre o que você deseja perguntar?", ["URL", "PDF"])

if data_source == "URL":
    query_url = st.text_input('Faça sua pergunta sobre URLs:')
    if query_url:
    ...
    query_pdf = st.text_input('Faça sua pergunta sobre PDFs:')
    if query_pdf:
