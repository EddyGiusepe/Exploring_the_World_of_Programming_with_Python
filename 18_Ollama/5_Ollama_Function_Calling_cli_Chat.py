...
8b-instruct-fp16
================
a8f4d8643bb2 • 16GB • Updated 2 weeks ago
"""
import ollama
import requests
...
def chat_with_ollama_no_functions(user_question):
    response = ollama.chat(
        model="'llama3.1:8b-instruct-fp16'", # ollama pull llama3.1:8b (NÃO É BOM!!!)               #'llama3.1:8b-instruct-fp16' llama3.1:8b
        messages=[
...
def chat_with_ollama(user_question):
    response = ollama.chat(
        model='llama3.1:8b-instruct-fp16', #'llama3.1:8b-instruct-fp16'
        messages=[
            {'role': 'user', 'content': user_question}
        ],
        tools=[
